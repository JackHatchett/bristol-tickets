---
name: base-resume-update
description: Changes what the master resume itself says, so the change carries into every application made from it afterwards. Use when a new job, achievement or skill should go on the record with no particular posting in view.
license: MIT
metadata:
...
`src/skills/resume-tailoring/SKILL.md` owns JD-driven output and the formatted
and plain-text conventions both files rely on; nothing here produces a tailored
document.

## Preconditions

- **Run `src/skills/resume-tailoring/SKILL.md` instead where a job description is in play.** A
