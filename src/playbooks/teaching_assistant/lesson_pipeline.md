# lesson_pipeline — teaching_assistant playbook
...
**Generate the three files from the plan**, per `content_generation.md`, which
...
and the hand-off follows `protocols/teaching_assistant/copilot_bridge.md`;
...
`html_render.md`). Stray markup survives both a writer and a linter, so this
...
- `content_generation.md` — stage 2 execution detail.
