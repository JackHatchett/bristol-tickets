...
charter's standard roadmap-db check, which happens first (per the
charter's §2.1).
...
1. After the standard roadmap-db check (including backlog cards assigned to
   ...
   context.
2. The roadmap-db check already surfaced that project's epic (name,
...
1. Update the project's roadmap epic: `status`/`next_action` if the phase or
   immediate focus changed, via `roadmap_write.py` (there is no
...
3. If a decision resolves an open task, close it via `roadmap_write.py
...
project — the roadmap db is the default; a project-local state file is the
