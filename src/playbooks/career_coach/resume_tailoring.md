# resume_tailoring — career_coach playbook
...
`jd_evaluation.md` §The fit rubric for the split. **The ATS source of truth is
...
- **Never flag a keyword as MISSING where a context file documents it as real
  ...
  status, per `jd_evaluation.md`'s rubric.
...
- **Top ATS keywords** — the 10 to 15 most important, each checked against both
  ...
  user correction to a `[MISSING]` flag for `session_closure.md`** rather than
...
`session_closure.md` or on demand.
