# migrate_legacy_agent — Playbook

Convert a legacy agent bundle — a pre-framework folder of instructions and
tools — into this framework's pattern: a charter under
`src/agent_identities/`, machinery split across playbooks, tools and protocols,
and personal data relocated out of `/src`. For an agent that does not exist yet,
use `src/playbooks/create_agent.md` instead.
...
- **`src/playbooks/<agent>/`** — procedures a session follows end to end.
- **`src/protocols/<agent>/`** — instructions for coordinating with an external
  party on a specific task. `src/templates/protocol_template.md` draws the line.
