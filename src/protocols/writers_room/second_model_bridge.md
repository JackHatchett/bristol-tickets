...
- **Guardrails cited, never restated:** `src/agent_identities/writers_room.md`
  holds the hard rules; `playbooks/writers_room/story_proposals.md` is how a
  return is filed; `playbooks/writers_room/voice_distillation.md` holds the
...
- **The zero-context reader test** (`manuscript_review.md`). A session that has
...
- **A scouting pass over a corpus** (`voice_distillation.md`), where the sample
...
- **Every delta, flag or specimen is a proposal**, reconciled and filed per
  `story_proposals.md` or distilled per `voice_distillation.md`. **Well-formed
  is not accepted.**
...
- `playbooks/writers_room/manuscript_review.md`,
  `playbooks/writers_room/voice_distillation.md` — the capabilities that reach
  for this bridge.
